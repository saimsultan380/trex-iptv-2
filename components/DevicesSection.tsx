...
import { Flame, Smartphone, Box, Tv, Radio, Apple, Monitor, HardDrive, Settings2 } from "lucide-react";
...
const apps = [
  {
    ...
    icon: <Tv className="h-4 w-4" />,
    list: ["Ibo TV Player", "Ibo Pro Player", "Hot IPTV", "Smart One IPTV", "Bob Player", "MyTV Online", "IPTV Smarters Pro"],
  },
  {
    category: "iOS Apps",
    icon: <Apple className="h-4 w-4" />,
    list: ["iPlayTV", "Ibo Player Pro", "GSE Smart IPTV", "VU IPTV Player"],
  },
  {
    category: "Windows / macOS Apps",
    icon: <Monitor className="h-4 w-4" />,
    list: ["Ibo Player", "IPTV Expert", "VLC Media Player", "Kodi"],
  },
  {
    category: "Android Apps",
    icon: <Smartphone className="h-4 w-4" />,
    list: ["IPTV Smarters Pro", "TiviMate", "Ibo Player", "GSE Smart IPTV", "Perfect Player"],
  },
];
  ...
  return (
    <section id="devices" className="mx-auto max-w-7xl w-full px-6 py-12 sm:px-8 sm:py-16 lg:py-24 bg-white">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 lg:gap-6 items-start mb-8 sm:mb-14">
        ...
        {/* Left Column — badge, heading, desc, device list */}
        <motion.div
        ...
        >
          ...
          <span className="text-[#ff6b35] text-[10px] sm:text-xs font-bold tracking-widest uppercase mb-2 flex items-center gap-1.5">
            <Tv className="h-3.5 w-3.5" /> TREX IPTV COMPATIBLE DEVICES &amp; APPS
          </span>

          <h2 className="text-[26px] sm:text-4xl lg:text-[42px] font-bold tracking-tight leading-tight text-zinc-900 mb-3 sm:mb-4">
            <span className="text-[#ff6b35]">Trex IPTV</span> Works on Every Device You Own
          </h2>

          <p className="text-zinc-500 text-[12px] sm:text-[14px] leading-relaxed mb-5 sm:mb-8 max-w-sm">
            Whether you are at home or on the go, Trex IPTV runs smoothly across all your screens without any complicated setup.
          </p>

          {/* Device list - 2 per row */}
          <div className="grid grid-cols-2 gap-x-3 gap-y-3 sm:gap-x-4 sm:gap-y-4">
            {devices.map((device, i) => (
              ...
              >
                ...
                <div>
                  <h4 className="text-[12px] sm:text-[13px] font-bold text-zinc-900 leading-tight">{device.name}</h4>
                  <p className="text-[10px] sm:text-[11px] text-zinc-400 leading-snug">{device.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        >
          <div className="relative w-full aspect-[1402/1000] max-w-[260px] sm:max-w-none mx-auto lg:mx-0">
            <Image
              ...
              alt="Trex IPTV Supported Devices"
              fill
          ...
          {apps.map((app, i) => (
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-[11px] sm:text-[13px] font-bold text-zinc-900">{app.category}</h4>
              </div>
