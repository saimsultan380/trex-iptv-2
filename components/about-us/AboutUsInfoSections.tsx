    ...
    <SectionBlock title="What We" accent="Provide">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        {whatWeProvide.map((paragraph, index) => (
          <p
            key={index}
            className="text-zinc-600 text-[13px] sm:text-[15px] leading-relaxed font-medium glass-card px-4 py-4 h-full"
          >
            {paragraph}
          </p>
        ))}
...
export function ResponsibleUseSection() {
  return (
    <SectionBlock title="Responsible" accent="Use">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        <p className="text-zinc-600 text-[13px] sm:text-[15px] leading-relaxed font-medium glass-card px-4 py-4 flex items-start gap-3 h-full">
          <Shield className="h-5 w-5 text-[#ff3503] shrink-0 mt-0.5" />
          Customers must use the service in accordance with applicable laws and account rules.
        </p>
        <p className="text-zinc-600 text-[13px] sm:text-[15px] leading-relaxed font-medium glass-card px-4 py-4 h-full">
